...
public class ClientModel {
	...
	public ClientModel(BusinessLocation location) {
		...
		final MessageHandler messageHandler = new ClientMessageHandler(this);
	...
	public boolean login(CustomerLogin login) {
		final ObjectMessage tx = new ObjectMessage(Message.Command.SUBMIT_LOGIN);
	...
	public boolean register(Customer customer) {
		final ObjectMessage tx = new ObjectMessage(Message.Command.REGISTER_NEW_CUSTOMER);
		tx.addObject("CUSTOMER", customer);
		return comms.sendMessage(tx);
	}

	/**
	 * Send a customer modification request to the business, attempt to modify the currently logged
	 * in customer.
	 *
	...
	public boolean modifyCustomer(Customer modified) {
		final ObjectMessage tx = new ObjectMessage(Message.Command.MODIFY_CUSTOMER);
	...
	/**
	 * Logout the current customer; this results in all server objects being cleared so that any
	 * existing objects do not persist between different users.
	 */
	public void logout() {
		// Reset statuses
		orderResponse.clear();
	...
	public boolean refreshPostcodes() {
		...
		final Message tx = new Message(Message.Command.GET_POSTCODES);
		return comms.sendMessage(tx);
	}
	...
	public boolean refreshDishes() {
		...
		final Message message = new Message(Message.Command.GET_DISH_STOCK);
		return comms.sendMessage(message);
	}

	/**
	 * Send an existing order update request to the business. Use the currently logged in customer
	 * for filtering.
	 *
	 * @return Whether message successfully sent
	 */
	public boolean refreshOrders() {
		...
		final ObjectMessage tx = new ObjectMessage(Message.Command.GET_EXISTING_ORDERS);
		tx.addObject("CUSTOMER_LOGIN", loggedInCustomer.readObject().getLogin());
		return comms.sendMessage(tx);
	}

	/**
	 * Send a new order to the business using a list of dishes. Order comes from the currently
	 * logged in customer.
	 *
	...
	public boolean sendOrder(QuantityMap<Dish> dishes) {
		final Order order = new Order(loggedInCustomer.readObject(), dishes);
		final ObjectMessage tx = new ObjectMessage(Message.Command.SUBMIT_ORDER);
