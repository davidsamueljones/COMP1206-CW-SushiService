...
public class NewOrderPanel extends JPanel implements View {
	...
	public NewOrderPanel(ClientApplication application) {
		...
		// [LHS] - Dishes
		// [LHS] <- 'Toolbar'
		final ToolBar tlbDishes = new ToolBar(getBackground(), Color.BLACK, Color.WHITE);
		...
		final ToolBarButton tbbRefresh = tlbDishes.addButton("Refresh",
		...
		final JButton btnMinusOne = new JButton("-1 from Order");
		btnMinusOne.setEnabled(false);
		final GridBagConstraints gbc_btnMinusOne = new GridBagConstraints();
		...
		final JButton btnPlusOne = new JButton("+1 to Order");
		btnPlusOne.setEnabled(false);
		final GridBagConstraints gbc_btnPlusOne = new GridBagConstraints();
		...
		final JButton btnRemoveAll = new JButton("Remove From Order");
		...
		final JPanel pnlLHS = new JPanel(new BorderLayout());
		...
		// [RHS] - Order
		// [RHS] <- 'Toolbar'
		final ToolBar tlbOrder = new ToolBar(getBackground(), Color.BLACK, Color.WHITE);
		...
		tlbOrder.addSeparator();
		final ToolBarButton tbbSubmitOrder = tlbOrder.addButton("Submit Order",
		...
		// [RHS] <- 'Order' Panel
		final JPanel pnlOrder = new JPanel();
		final GridBagLayout gbl_pnlOrder = new GridBagLayout();
		...
		Utilities.setColumnStringFormat(tblOrder, 2, Utilities.PRICE_FORMAT, SwingConstants.RIGHT);
		// Make scrollable
		final JScrollPane scrOrder = new JScrollPane(tblOrder);
		scrOrder.setPreferredSize(new Dimension(0, 0));
		scrOrder.setBackground(pnlOrder.getBackground());
		final GridBagConstraints gbc_scrOrder = new GridBagConstraints();
		...
		final GridBagConstraints gbc_nudTotalPrice = new GridBagConstraints();
		...
		final JPanel pnlRHS = new JPanel(new BorderLayout());
		...
		final JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, pnlLHS, pnlRHS);
		...
		tblDishes.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (!e.getValueIsAdjusting()) {
					final Quantity<Dish> selection =
							model_tblDishes.getSelectedTableObject(tblDishes);
					if (selection == null) {
						btnMinusOne.setEnabled(false);
						btnPlusOne.setEnabled(false);
						btnRemoveAll.setEnabled(false);
		...
		btnMinusOne.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				final Dish dish = model_tblDishes.getSelectedTableObject(tblDishes).getItem();
				final Double quantity = orderItems.get(dish);
		...
		tbbSubmitOrder.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (application.model.sendOrder(orderItems)) {
					...
					if (application.model.orderResponse
							.waitForNew(ClientApplication.REQUEST_TIMEOUT)) {
						...
						if (!eb.isError()) {
							...
							res = JOptionPane.showConfirmDialog(null,
									...
									"Order Successful", JOptionPane.YES_NO_OPTION,
				...
				if (eb.isError()) {
					JOptionPane.showMessageDialog(null, eb.listComments("Order Failed"),
	...
	private void refreshOrder() {
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				final Object selection = model_tblOrder.getSelectedTableObject(tblOrder);
	...
	/**
	 * Verify that current order being created does not have any out of date information based on a
	 * collection of available dishes.
	 *
	...
	private void verifyCurrentOrder(Collection<Dish> dishes) {
		...
		final Iterator<Dish> itrOrder = orderItems.keySet().iterator();
		while (itrOrder.hasNext()) {
			final Dish orderDish = itrOrder.next();
			// Check if dish is still in available dishes
			if (dishes.contains(orderDish)) {
				final Dish dish = Utilities.getCollectionItem(dishes, orderDish, Dish.class);
				...
				eb.addError(String.format("'%s' no longer exists - removed from basket",
						orderDish.getName()));
				itrOrder.remove();
			}
		}
		...
		if (eb.isError()) {
			refreshOrder();
			JOptionPane.showMessageDialog(null, eb.listComments("Dish Change Alert"),
					"Dish Change Alert", JOptionPane.INFORMATION_MESSAGE);
		}
	}
